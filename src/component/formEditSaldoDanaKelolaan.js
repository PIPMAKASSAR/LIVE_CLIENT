...
import { clearSaldoDanaKelolaanStatus } from "../redux/feature/saldoDanaKelolaanSlice";
...
export default function FormEditSaldoDanaKelolaan () {
    const MySwal = withReactContent(Swal)
    const data = useSelector(state => state.saldoDanaKelolaan.item)
    const dispatch = useDispatch()
    const statusSaldoDanaKelolaan = useSelector(state => state.saldoDanaKelolaan.edit)
    ...
    const handleEdit = (event) => {
        ...
        dispatch(saldoDanaKelolaanApi.putSaldoDanaKelolaan(payload))
        .then(() => {
            setIsLoading(false)
            MySwal.fire({
                icon: "success",
                title: "Data Saldo Dana Kelolaan Berhasil Di Edit",
            })
        })
        .catch((err) => {
            setIsLoading(false)
            MySwal.fire({
                icon: "error",
                title: "Gagal Mengedit Data Saldo Dana Kelollan",
              });
        })
    }
